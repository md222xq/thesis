...
function CashOut(uint256 varg0) public nonPayable { 
    if (!bool(varg0 > _balances[address(address(msg.sender))])) {
        ...
        if (v0) {
            ...
            v1 = stor_3_0_19.AddMessage(msg.sender, varg0, 'CashOut').gas(msg.gas);
...
function Deposit() public payable { 
    if (!bool(msg.value < _minDeposit)) {
        ...
        require(bool(stor_3_0_19.code.size));
        v0 = stor_3_0_19.AddMessage(msg.sender, msg.value, 'Deposit').gas(msg.gas);
...
function setLog(address varg0) public nonPayable { 
...
function __function_selector__(bytes4 function_selector) public payable { 
    ...
    if (msg.data.length >= 4) {
        if (0x11bcd830 == uint32(function_selector >> 224)) {
            ...
            balances(address);
        } else if (0x4797f21b == uint32(function_selector >> 224)) {
            setLog(address);
