...
// 2023.11.08 21:34 UTC
// Compiled using the solidity compiler version 0.4.7<=v<0.5.9


// Data structures and variables inferred from the use of storage instructions
uint256 _ownerCut; // STORAGE[0x2]
mapping (uint256 => struct_356) _cancelAuction; // STORAGE[0x3]
uint256 _owner; // STORAGE[0x0] bytes 0 to 19
uint256 _isSiringClockAuction; // STORAGE[0x4] bytes 0 to 0
uint256 _nonFungibleContract; // STORAGE[0x1] bytes 0 to 19
uint256 _paused; // STORAGE[0x0] bytes 20 to 20

struct struct_356 { uint256 field0; uint256 field1; uint256 field2; };
...
function 0x1216(uint256 varg0, address varg1) private { 
...
function 0x1325(uint256 varg0, uint256 varg1) private { 
...
function bid(uint256 varg0) public payable { 
    ...
    0x1216(varg0, address(_cancelAuction[varg0].field0));
}
...
function isSiringClockAuction() public nonPayable { 
    return _isSiringClockAuction;
}
...
function cancelAuctionWhenPaused(uint256 varg0) public nonPayable { 
    ...
    require(bool(address(msg.sender) == _owner));
    require(bool(uint64(uint64(_cancelAuction[varg0].field2 >> 64)) > 0));
    0x1325(address(_cancelAuction[varg0].field0), varg0);
}
...
function createAuction(uint256 varg0, uint256 varg1, uint256 varg2, uint256 varg3, address varg4) public nonPayable { 
    ...
    require(bool(address(msg.sender) == _nonFungibleContract));
    require(bool(_nonFungibleContract.code.size));
    v0 = _nonFungibleContract.transferFrom(varg4, address(this), varg0).gas(msg.gas);
    require(bool(v0), 0, RETURNDATASIZE()); // checks call status, propagates error data on error
...
function __function_selector__(bytes4 function_selector) public payable { 
    ...
    if (msg.data.length >= 4) {
        if (0x27ebe40a == uint32(function_selector >> 224)) {
            ...
            withdrawBalance();
        } else if (0x76190f8f == uint32(function_selector >> 224)) {
