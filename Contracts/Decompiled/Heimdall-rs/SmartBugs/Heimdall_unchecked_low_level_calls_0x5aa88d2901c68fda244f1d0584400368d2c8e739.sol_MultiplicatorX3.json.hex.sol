...
contract DecompiledContract {
    ...
    /// @custom:selector    0x1ac9f70d
    /// @custom:name        multiplicate
    /// @param              arg0 ["address", "bytes", "bytes20", "bytes32", "int", "int160", "int256", "string", "uint", "uint160", "uint256"]
    function multiplicate(address arg0) public payable {
        require(msg.value < (address(this).balance));
        (bool success, bytes memory ret0) = address(arg0).call{ gas: (!(address(this).balance) + msg.value) * 0x08fc, value: (address(this).balance) + msg.value }(abi.encode());
    }
